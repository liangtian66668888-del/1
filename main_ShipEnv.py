import numpy as np
from allocation_ShipEnv import Allocation,SelfishAllocation,AllocationWoPre,AllocationNormalCritic,AllocationNormal,ShipEnv
...
parser.add_argument('--model_dir', default="./models/ShipEnv_random")
        ...
        while not done:
            ...
            for idx, each in enumerate(allocation):
                if len(each) > 0 and avail_task[idx] == 1:
                    avail_task[idx] = 0
                if (not np.array_equal(state[env.task_dim * idx:env.task_dim * (idx + 1)],
                                       next_state[env.task_dim * idx:env.task_dim * (idx + 1)])) and (
                        avail_task[idx] == 0):
                    avail_task[idx] = 1
    ...
    while not done:
        ...
        for idx, each in enumerate(allocation):
            if len(each) > 0 and avail_task[idx] == 1:
                avail_task[idx] = 0
            if (not np.array_equal(state[env.task_dim * idx:env.task_dim * (idx + 1)],
                                   next_state[env.task_dim * idx:env.task_dim * (idx + 1)])) and (avail_task[idx] == 0):
                avail_task[idx] = 1
